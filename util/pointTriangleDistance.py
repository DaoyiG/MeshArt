...
# from https://github.com/autonomousvision/occupancy_networks/blob/406f79468fb8b57b3e76816aaa73b1915c53ad22/im2mesh/dmc/utils/pointTriangleDistance.py
...
def batch_point_triangle_distance_fast(triangles, points):
    """
    Calculate distance between a set of points and a set of triangles in 3D.
    Approximate method, clamp s and t to enable batch calculation.
    
    Args:
    - triangles: torch.Tensor of shape (M, 3, 3) representing M triangles.
    - points: torch.Tensor of shape (N, 3) representing N points.
    
    Returns:
    - distances: torch.Tensor of shape (N, M) representing distances between each point and each triangle.
    """
    assert not torch.isnan(triangles).any()
    assert not torch.isnan(points).any()
    
    B = triangles[:, 0]  # (M, 3)
    E0 = triangles[:, 1] - B  # (M, 3)
    E1 = triangles[:, 2] - B  # (M, 3)

    D = B.unsqueeze(1) - points.unsqueeze(0)  # (M, N, 3)

    d = torch.einsum('mij,mj->mi', D, E0)  # (M, N)
    e = torch.einsum('mij,mj->mi', D, E1)  # (M, N)
    f = torch.einsum('mij,mij->mi', D, D)  # (M, N)
    f = f.unsqueeze(1)
    f = torch.diagonal(f, dim1=1, dim2=2).unsqueeze(1)  # (M, 1)

    a = torch.sum(E0 * E0, dim=1).unsqueeze(1).expand_as(d)  # (M, N)
    b = torch.sum(E0 * E1, dim=1).unsqueeze(1).expand_as(d)  # (M, N)
    c = torch.sum(E1 * E1, dim=1).unsqueeze(1).expand_as(d)  # (M, N)

    det = a * c - b * b
    eps = torch.finfo(det.dtype).eps
    s = (b * e - c * d) / (det + eps)
    t = (b * d - a * e) / (det + eps)
    ...
    norm = torch.clamp(s + t, min=eps)
    s = s / norm
    t = t / norm

    sqrdistance = s * (a * s + b * t + 2.0 * d) + t * (b * s + c * t + 2.0 * e) + f

    return torch.clamp(sqrdistance, min=0)
...
if __name__ == '__main__':
    ...
    dists2 = batch_point_triangle_distance_fast(TRI.unsqueeze(0), P)
    print(dists)
    print(dists2)

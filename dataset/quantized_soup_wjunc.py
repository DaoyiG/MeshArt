...
from dataset.quantize_and_tokenize_faces import get_shifted_sequence
from dataset.triangles_structure import create_feature_stack
...
class QuantizedSoupCreator(torch.nn.Module):
    ...
    def get_completion_sequence(self, x, edge_index, face, num_vertices, tokens):
        ...
        target = target[0]
        # If tokens is None or a negative value, return the full sequence
        if tokens is None or tokens < 0:
            num_pre_tokens = len(soup_sequence)
    ...
    @torch.no_grad()
    def decode(self, sequence, decoder, fast_recon=False):
        if fast_recon:
        ...
        decoded_x_conv, decoded_x_junc = decoder(encoded_x_conv)
        decoded_x = decoded_x_conv.reshape(-1, decoded_x_conv.shape[-2], decoded_x_conv.shape[-1])[conv_mask, :, :]
        coords = decoded_x.argmax(-1).detach().cpu().numpy() / self.num_tokens - 0.5
        gen_vertices, gen_faces = triangle_sequence_to_mesh(coords)
        ...
        return gen_vertices, gen_faces, gen_juncs
